...
export default function App() {
  ...
  useEffect(() => {
    const initApp = async () => {
      try {
        // Cập nhật chuẩn xác thực bắt buộc của hệ thống
        ...
        // Thiết lập Status Bar cho Native App
        try {
        ...
        // Yêu cầu quyền Web Notification (Dự phòng Web)
  ...
  useEffect(() => {
    ...
    try {
      if (window.Capacitor && window.Capacitor.Plugins.App) {
        ...
        // Sửa lỗi e.then is not a function: Xử lý an toàn cho cả Capacitor cũ và mới
    ...
    return () => {
      // Dọn dẹp an toàn khi component bị hủy (ví dụ: chuyển từ login sang main)
  ...
  useEffect(() => {
    ...
    // Thêm hàm bắt lỗi (error callback) để tránh app crash trắng màn hình
    const unsubUsers = onSnapshot(getUsersRef(), (snap) => {
      setAllUsers(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    }, (err) => console.error("Lỗi tải users:", err));

    // BỎ orderBy của Firebase, thay bằng kéo về rồi tự sort bằng Javascript
    const unsubMsgs = onSnapshot(getMessagesRef(), (snap) => {
      let msgs = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      msgs.sort((a, b) => a.timestamp - b.timestamp); // Sort trong memory
  ...
  }, [firebaseUser, myProfile, activePartner]);
  ...
  return (
    // Background thay đổi dựa trên screen để Status Bar trên iOS/Android luôn đồng màu (Tím Yahoo hoặc màu tối)
    <div className={`h-[100dvh] w-full font-sans flex items-center justify-center relative overflow-hidden md:p-6 lg:p-12 transition-colors duration-300 ${currentScreen === 'login' || currentScreen === 'register' ? 'bg-[#771285]' : 'bg-[#681a75] md:bg-[#2e3440]'}`}>
      <style dangerouslySetInnerHTML={{
        ...
        /* Bong bóng chat chuẩn form, màu sắc mềm mại */
        .bubble-me { 
        ...
        .btn-classic-buzz:active { transform: scale(0.96); box-shadow: inset 0 2px 4px rgba(0,0,0,0.4); background: #e6a200; }
        ...
        /* Cải tiến rung toàn màn hình chat */
      ...
      {/* Container của App - Responsive thành Window trên PC, chiếm Full trên Mobile */}
      <div className="w-full h-full md:max-h-[850px] md:max-w-[1200px] md:rounded-2xl md:shadow-[0_20px_60px_rgba(0,0,0,0.6)] bg-white flex flex-col relative overflow-hidden md:border border-gray-600">
        ...
        {currentScreen === 'login' || currentScreen === 'register' ? (
          ...
          <div className="flex-1 flex overflow-hidden w-full h-full relative">
            ...
            <div className={`w-full md:w-[340px] lg:w-[380px] h-full flex flex-col bg-[#f8fafc] shrink-0 md:border-r border-gray-300 transition-transform duration-300 ease-in-out ${activePartner ? '-translate-x-full md:translate-x-0 absolute md:relative z-10 md:z-auto' : 'translate-x-0'}`}>
              <FriendListScreen
            ...
            {/* CỘT PHẢI: Khung Chat */}
            <div className={`flex-1 h-full flex flex-col bg-[#eef1f6] overflow-hidden transition-transform duration-300 ease-in-out ${!activePartner ? 'translate-x-full md:translate-x-0 absolute md:relative w-full z-20 md:z-auto' : 'translate-x-0 w-full z-20 md:z-auto'}`}>
              {activePartner ? (
                <ChatScreen
...
// --- FRIEND LIST SCREEN ---
function FriendListScreen({ myProfile, allUsers, allMessages, lastRead, db, onLogout, onChat, activePartner }) {
  const [showAddModal, setShowAddModal] = useState(false);
  const [searchId, setSearchId] = useState('');

  const myFriendsIds = myProfile?.friends || [];
  const myFriends = allUsers.filter(u => myFriendsIds.includes(u.username));
  const displayList = [PUBLIC_ROOM, ...myFriends];
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#f8fafc] relative w-full">
      {/* HEADER CỐ ĐỊNH - Tích hợp padding an toàn cho tai thỏ */}
              ...
              <div key={user.id} onClick={() => onChat(user)} className={`px-4 py-3 flex items-center gap-3.5 transition-colors cursor-pointer group ${isSelected ? 'bg-purple-50' : 'bg-white hover:bg-gray-50'}`}>
                ...
                <div className="flex-1 flex flex-col min-w-0 justify-center">
                  <div className="flex justify-between items-center mb-0.5">
                    ...
                    {unread > 0 && <span className="bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm animate-pulse">{unread}</span>}
                  </div>
...
// --- CHAT SCREEN ---
function ChatScreen({ myProfile, chatPartner, allMessages, db, bgClass, onCycleBg, onBack }) {
  ...
  useEffect(() => {
    ...
    if (chatMessages.length > 0) {
      ...
      if (lastMsg.text === 'BUZZ!!!' && lastMsg.sender !== myProfile?.username && !isBuzzing) triggerBuzz();
    }
  }, [chatMessages.length]);
  ...
  return (
    // Animation Buzz được áp dụng lên cả khung chat này
    <div className={`flex-1 flex flex-col h-full relative transition-colors duration-500 ${bgClass} ${isBuzzing ? 'animate-buzz' : ''} w-full`}>
      ...
      <div className="bg-yahoo-header px-3 pb-3 pt-[calc(env(safe-area-inset-top)+0.75rem)] flex items-center justify-between z-20 shrink-0 shadow-md">
        ...
        <div className="flex flex-col items-center flex-1 min-w-0 px-2">
          ...
          <span className="text-purple-200 text-[11px] font-medium mt-0.5">Online</span>
        </div>
      ...
      {/* KHU VỰC TIN NHẮN CUỘN ĐƯỢC - Xây dựng lại Flexbox chuẩn */}
      <div className={`flex-1 overflow-y-auto p-4 flex flex-col gap-3 z-10 scroll-smooth`}>
        ...
        {chatMessages.map((msg, index) => {
          ...
          if (msg.text === 'BUZZ!!!') return null;

          // Kiểm tra xem tin nhắn trước đó có cùng người gửi không để nhóm lại (tùy chọn)
          ...
          return (
            <div key={msg.id} className={`flex w-full ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex flex-col max-w-[75%] md:max-w-[65%] ${isMe ? 'items-end' : 'items-start'}`}>
                ...
                {/* Tên người gửi nếu là phòng chung */}
      ...
      {/* THANH NHẬP LIỆU CỐ ĐỊNH PHÍA DƯỚI - Nâng cấp giao diện Modern */}
      <div className="bg-white border-t border-gray-200 px-3 pt-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] flex items-end gap-2.5 z-20 shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.03)]">
        ...
        <div className="flex items-center gap-2 mb-1 shrink-0">
          <button onClick={() => { handleSend("BUZZ!!!"); triggerBuzz(); }} className="h-10 px-4 btn-classic-buzz flex items-center justify-center text-[14px] uppercase tracking-wider">BUZZ</button>
